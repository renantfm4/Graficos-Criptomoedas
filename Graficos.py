...
#Ler .csv
df = pd.read_csv('/Users/benjamim/PycharmProjects/DashAPC/venv/data/coin_Bitcoin.csv')
df_array = df.values
...
df = pd.read_csv('/Users/benjamim/PycharmProjects/DashAPC/venv/data/coin_Bitcoin.csv')
df1 = pd.read_csv('/Users/benjamim/PycharmProjects/DashAPC/venv/data/O_brabo.csv')
...
df_moedas = pd.read_csv('/Users/benjamim/PycharmProjects/DashAPC/venv/data/Criptomoedas.csv')
...
moedas = pd.DataFrame(valores_media, columns=['Media Preco Dia','Moeda', 'Data'])
...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
        ...
        dcc.Dropdown(
            id='dropdown',
            options=[{'label': i, 'value': i} for i in avaliable_names], value=None, multi=False,
            placeholder='Filtre as moedas'
        )
    ]),
    html.Div(children=[
        dcc.Graph(
...
def atualizar_output(value):
    if value != None:
        ...
        fig1.update_layout(xaxis=dict(title="Anos"),
                           yaxis=dict(title="Média anual"))

        return fig1
    else:
        fig2.update_layout(xaxis=dict(title="Anos"),
                           yaxis=dict(title="Média anual"))
        return fig2
...
def atualizar_output(value):
    if value == 'Gráfico Dólar':
        plot1.update_layout(xaxis=dict(title="Anos"),
                           yaxis=dict(title="Valor"))
        return plot1
    elif value == 'Gráfico Bitcoin':
        plot2.update_layout(xaxis=dict(title="Anos"),
                           yaxis=dict(title="Valor"))
        return plot2
...
def update_volume(value):
    ...
    if value == None:
        graph_volume.update_layout(xaxis=dict(title="Anos"),
                           yaxis=dict(title="Valor do volume"))
        return graph_volume
    fig.update_layout(xaxis=dict(title="Anos"),
                       yaxis=dict(title="Valor Volume"))
    return fig
...
def update_volume(selected_value):
    ...
    fig.update_layout(xaxis=dict(title="Anos"),
                       yaxis=dict(title="Marketcap Arrecadado"))
    return fig
    ...
    if value == 'Real':
        ...
        for coluna in media_preco:
            valores_media.append(
                [((coluna[4] + coluna[5]) / 2) * valores[valores_2.index(value)], coluna[2], coluna[3]])
